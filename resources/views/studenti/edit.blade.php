...
@section('content')
    ...
    <form method="POST" action="{{ route('studenti.update', $student->id) }}">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="nome" class="block">Nome</label>
            <input type="text" name="nome" id="nome" value="{{ $student->nome }}" class="border p-2 w-full" required>
        </div>

        <div class="mb-4">
            <label for="cognome" class="block">Cognome</label>
            <input type="text" name="cognome" id="cognome" value="{{ $student->cognome }}" class="border p-2 w-full" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="classe_id" id="classe_id" class="border p-2 w-full" required>
                @foreach ($classes as $classe)
                    <option value="{{ $classe->id }}" {{ $student->classe_id == $classe->id ? 'selected' : '' }}>
